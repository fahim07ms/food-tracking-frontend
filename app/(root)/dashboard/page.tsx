...
export default function DashboardPage() {
    ...
    const todayLogs = userData?.foodLogs?.filter((log: any) => {
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {userData?.healthProfile?.current_weight_kg || "--"} kg
                        </div>
                        <p className="text-xs text-muted-foreground">
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold capitalize">
                            {currentGoal?.primary_goals?.[0]?.replace("_", " ") || "None"}
                        </div>
                        <p className="text-xs text-muted-foreground">
                            Active Goal
                        </p>
                                    ...
                                    <div key={i} className="flex items-center">
                                        <div className="ml-4 space-y-1">
                                            <p className="text-sm font-medium leading-none">
                                                {typeof log.foodItem === 'object' && log.foodItem?.name
                                                    ? log.foodItem.name
                                                    : `Food Item ID: ${log.foodItem}`}
                                            </p>
                                            <p className="text-sm text-muted-foreground">
                                                {log.time} - {log.quantity} portion(s)
                                            </p>
                                        </div>
